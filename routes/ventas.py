...
def eliminar_venta(venta_id):
    """Elimina una venta completa y libera la mesa."""
...
# =========================================================
# TERMINAL DE VENTA (UI PRINCIPAL)
# =========================================================
...
def ver_mesa(mesa_id):
    ...
    # Si no existe venta abierta, crear una nueva
    if not venta:
    ...
    # Si se cambia el nombre cliente
    elif nombre_cliente:
...
def agregar_producto():
    ...
    try:
        ...
        precio = (
            producto.valor_interno
            if cliente and cliente.tipo == "premium"
            else producto.valor_venta
        )

        # Restar stock
        producto.cantidad -= 1

        # Buscar si ya existe en detalle
        detalle = VentaDetalle.query.filter_by(
            venta_id=venta.id,
            producto_id=producto.id
        ).first()
        ...
        # Recalcular total
        venta.total = sum(
            d.subtotal for d in VentaDetalle.query.filter_by(venta_id=venta.id).all()
        )
...
def actualizar_cantidad():
    ...
    try:
        ...
        return jsonify({
            ...
            "nuevo_subtotal": detalle.subtotal
        })
...
def cerrar_venta():
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "redirect_url": url_for("ventas.ver_ticket", venta_id=venta.id)
        })
